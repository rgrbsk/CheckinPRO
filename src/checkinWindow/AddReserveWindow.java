...
public class AddReserveWindow {
	...
	public void initialize() {
		...
		panelCheckDate
				.setBorder(new CompoundBorder(new EtchedBorder(EtchedBorder.LOWERED, null, null), new TitledBorder(null,
						"Conferir Datas", TitledBorder.CENTER, TitledBorder.TOP, null, new Color(187, 187, 187))));
	...
	public void roomTable() {
		...
		try (ResultSet rs = Conexao.executeQuery(sql, params)) {
			while (rs != null && rs.next()) {
				Object[] row = { rs.getInt("id"), rs.getInt("nmr_quarto"), rs.getInt("andar"), rs.getInt("camas"),
						"R$" + rs.getInt("preco_diaria"), rs.getString("tipo") };
				result.add(row);
		...
		if (model.getRowCount() == 0) {
			JOptionPane.showMessageDialog(frameAddReserve, " Sem quartos disponível neste período!", "Não disponível",
	...
	public boolean dateChecker() {
		try {
			...
			if (arrivalDate == null || departureDate == null) {
				JOptionPane.showMessageDialog(frameAddReserve, "Selecione ambas as datas", "Erro",
						JOptionPane.ERROR_MESSAGE);
				return false;
			}

			if (departureDate.before(arrivalDate)) {
				JOptionPane.showMessageDialog(frameAddReserve, "Data de saída deve ser após data de entrada", "Erro",
						JOptionPane.ERROR_MESSAGE);
				return false;
			}
			...
			if (available) {
				JOptionPane.showMessageDialog(frameAddReserve, "Quarto disponível neste período!", "Disponível",
						JOptionPane.INFORMATION_MESSAGE);
				return true;
			} else {
				JOptionPane.showMessageDialog(frameAddReserve, "Quarto não disponível nas datas selecionadas",
						"Indisponível", JOptionPane.WARNING_MESSAGE);
				return false;
			}
		} catch (NumberFormatException e) {
			JOptionPane.showMessageDialog(frameAddReserve, "Número do quarto inválido", "Erro",
					JOptionPane.ERROR_MESSAGE);
			return false;
		}
	}
	...
	public int userChecker() {
		try {
			...
			if (clientResultId != null && clientResultId > 0) {
				...
				JOptionPane.showMessageDialog(frameAddReserve, "Cliente não encontrado", "Erro",
						JOptionPane.WARNING_MESSAGE);
				return 0;
			}
		} catch (Exception e) {
			JOptionPane.showMessageDialog(frameAddReserve, "Erro ao verificar cliente: " + e.getMessage(), "Erro",
					JOptionPane.ERROR_MESSAGE);
			return 0;
		}
	}
	...
	public void createReserve() {
		...
		try {
			...
			int resultReservation = Reserve.adicionarReserva(IdQuarto, id, formattedArrival, formattedDeparture,
					roomcapacity, valorQuarto);

			if (resultReservation > 0) {
				JOptionPane.showMessageDialog(frameAddReserve, "Reserva cadastrada com sucesso!", "Sucesso",
						JOptionPane.INFORMATION_MESSAGE);
				frameAddReserve.dispose();

			} else {
				JOptionPane.showMessageDialog(frameAddReserve, "Falha ao cadastrar reserva", "Erro",
						JOptionPane.ERROR_MESSAGE);
			}
		} catch (Exception e) {
			JOptionPane.showMessageDialog(frameAddReserve, "Erro ao processar reserva: " + e.getMessage(), "Erro",
					JOptionPane.ERROR_MESSAGE);
			e.printStackTrace();
		}
	}
