...
public class Client {
	...
	public String toString() {
		return "Client [id=" + id + ", nome=" + nome + ", sobrenome=" + sobrenome + ", cpf=" + cpf + ", email=" + email
				+ ", telefone=" + telefone + "]";
	}
	...
	public static int cadastrarCliente(String nome, String sobrenome, String cpf, String email, String telefone) {
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "Erro ao cadastrar cliente: " + e.getMessage(), "Erro",
	...
	public static int atualizarCliente(int id, String nome, String sobrenome, String cpf, String telefone,
			String email) {
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "Erro ao atualizar cadastro: " + e.getMessage(), "Erro",
					JOptionPane.ERROR_MESSAGE);
			e.printStackTrace();
			return 0;
		}
	...
	public static List<Client> loadGridClientes() {
		...
		try (ResultSet rs = (ResultSet) Conexao.executeQuery(sql, parametros)) {
			while (rs.next()) { // ✅ Itera sobre todos os clientes
				Client cliente = new Client(rs.getInt("id_cliente"), rs.getString("nome"), rs.getString("sobrenome"),
	...
	public static List<Client> buscarClientes(String filtro, String valor, String limitarRegString) {
		...
		try (Connection conn = Conexao.getConnection()) {
			...
			try (PreparedStatement stmt = conn.prepareStatement(sql)) {
				...
				while (rs.next()) {
					clientes.add(new Client(rs.getInt("id_cliente"), rs.getString("nome"), rs.getString("sobrenome"),
							rs.getString("cpf"), rs.getString("email"), rs.getString("telefone")));
				}
			}
	...
	public static Client buscarClientePorId(int idCliente) {
		...
		try (Connection conn = Conexao.getConnection()) {
			...
			try (PreparedStatement stmt = conn.prepareStatement(sql)) {
				...
				if (rs.next()) {
					cliente = new Client(rs.getInt("id_cliente"), rs.getString("nome"), rs.getString("sobrenome"),
							rs.getString("cpf"), rs.getString("email"), rs.getString("telefone"));
				}
			}
		} catch (SQLException e) {
	...
	public static int excluirCliente(int clienteId) {
		...
		try {
			...
			if (rs.next() && rs.getInt(1) > 0) {
				JOptionPane.showMessageDialog(null, "Não é possível excluir. O cliente possui reservas ativas!",
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "Erro ao excluir cliente: " + e.getMessage(), "Erro",
					JOptionPane.ERROR_MESSAGE);
			e.printStackTrace();
			return 0;
		}
