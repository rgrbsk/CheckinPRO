	...
	public void addRoomDb() {
		try {
			...
			if (result > 0) {
				...
				JOptionPane.showMessageDialog(frameAddRoom, "Falha ao cadastrar quarto.", "Erro",
						JOptionPane.ERROR_MESSAGE);
			}

		} catch (NumberFormatException e) {
			JOptionPane.showMessageDialog(frameAddRoom, "Por favor, insira valores numéricos válidos.",
					"Erro de Formato", JOptionPane.ERROR_MESSAGE);
		} catch (Exception e) {
			JOptionPane.showMessageDialog(frameAddRoom, "Erro ao cadastrar quarto: " + e.getMessage(), "Erro",
					JOptionPane.ERROR_MESSAGE);
			e.printStackTrace();
		}
	}
